...
jQuery(document).ready(function ($) {
    ...
    $(".wpfd-content-preview").each(function () {
        ...
        if (preview_topCat == 'all_0') {
            preview_cParents[preview_topCat] = {parent: 0, term_id: 0, name: $(this).find("h2").text().trim()};
        } else {
            preview_cParents[preview_topCat] = {parent: 0, term_id: preview_topCat, name: $(this).find("h2").text().trim()};
        }

        $(this).find(".wpfdcategory.catlink").each(function () {
            var tempidCat = $(this).data('idcat');
            preview_cParents[tempidCat] = {parent: preview_topCat, term_id: tempidCat, name: $(this).text().trim()};
        });
    ...
    function preview_initClick() {
        $(document).off('click', '.wpfd-content-preview .catlink').on('click', '.wpfd-content-preview .catlink', function(e) {
            ...
            $(".wpfd-content[data-category=" + $(this).parents('.wpfd-content-'+ctheme).data('category') + "] .wpfd-container-"+ctheme).find('.wpfd-categories .wpfdcategory.catlink').each(function () {
                ...
                preview_cParents[tempidCat] = {parent: $(this).parents('.wpfd-content-'+ctheme).data('category'), term_id: tempidCat, name: catName};
            });
            e.preventDefault();
    ...
    function load(sourcecat, catid, page, upload_type) {
        ...
        window.wpfdAjax[preview_root_cat].file = $.ajax({
            ...
            cache: true,
            beforeSend: function () {
                if (wpfdPreviewFilesLocalCache.exist(previewFilesAjaxUrl) && (upload_type === null || upload_type === undefined)) {
        ...
        preview_tree.each(function () {
            ...
            setTimeout(function() {
                ele.find("li.directory a").each(function () {
                    ...
                    if (tempCatID == 'all_0') {
                        preview_cParents[tempCatID] = {parent: 0, term_id: 0, name: $(this).text().trim()};
                    } else {
                        preview_cParents[tempCatID] = {parent: patentCatID, term_id: tempCatID, name: $(this).text().trim()};
                    }
                });
            }, 1000);
        })
