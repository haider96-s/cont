...
class WpfdModelFts extends Model
{
        ...
        $dbscheme = array(
            'doc_content'    => array(
                'cols'   => array(
                    ...
                    'file_id'  => array('int(11)', 'NO', 'MUL'),
                    'content'  => array('longtext', 'NO', 'MUL')
                ),
                'index'  => array(
                    'PRIMARY'  => array(0, 'id'),
                    'file_id'  => array(1, 'file_id')
                ),
                'create' => 'CREATE TABLE `' . $this->getPrefix() . 'doc_content` (
    ...
    {
        /**
         * Filter to split the hyphen of document cleaner
         *
         * @return boolean
         *
         * @internal
         */
        $hyphenSplit = apply_filters('wpfd_fts_split_hyphen', true);
        if ($hyphenSplit) {
            // $str = str_replace('_', ' ', $str);
            $str = str_replace('-', ' ', $str);
        }

        $allowedChars = apply_filters('wpfd_plain_text_search_punct_to_exclude', array());
        $escapedAllowedChars = array_map('preg_quote', $allowedChars);
        ...
        $pattern_str = "~([\x{00C0}-\x{1FFF}\x{2C00}-\x{D7FF}\w" . implode('', $escapedAllowedChars) . "'][\x{00C0}" .
                       "-\x{1FFF}\x{2C00}-\x{D7FF}\w'\-]*[\x{00C0}-\x{1FFF}\x{2C00}-\x{D7FF}\w]+(?:[" . implode('', $escapedAllowedChars) . "][\x{00C0}-\x{1FFF}\x{2C00}-\x{D7FF}\w'\-]*[\x{00C0}-\x{1FFF}\x{2C00}-\x{D7FF}\w]+)*)~u";
     ...
     * @param integer $post_file_id Post file ID
     *
     * @return boolean
     */
    public function reindex($indexId, $chunks, $post_file_id = 0)
    {
        global $wpdb;
        ...
        foreach ($chunks as $key => $doc) {
            ...
            if ($key === 'post_title') {
                $r2 = $wpdb->query($wpdb->prepare('insert ignore into `' . $wpdb->prefix . 'wpfd_doc_content` (`file_id`, `content`) values ( %s, %s)', $post_file_id, $doc));
            } else {
                $r2 = $this->add(array($docId => $doc), $post_file_id);
            }
     ...
     * @return boolean
     */
    public function add($docs = array(), $fileID = 0)
    {
        global $wpdb;
        ...
        if ($wordlist !== '') {
            $query = 'insert ignore into `' . $wpdb->prefix . 'wpfd_doc_content` (`file_id`, `content`) values ("' . $fileID .'", "'. $wordlist . '")';
     ...
     /**
     * Reset wpfd_doc_content table
     *
     * @return boolean
     */
    public function resetDocContentTable()
    {
        global $wpdb;

        $table_wpfd_doc_content = $wpdb->prefix . 'wpfd_doc_content';
        $wpdb->query('TRUNCATE TABLE ' . $table_wpfd_doc_content);
     ...
     * @return boolean
     */
    public function addOld($docs = array())
    {
        global $wpdb;
    ...
    public function reindexPost($post_id, $is_force_remove = false)
    {
        ...
        if ($post && (!$is_force_remove)) {
            ...
            $this->clearLog();
            $res               = $this->reindex($insert_id, $chunks2, $post_id);
