...
//        }
    ...
    public function restoreicons()
    {
        ...
        $icons = array();
        $defaultIconPath = WPFD_PLUGIN_DIR_PATH . 'app/site/assets/icons/svg' . DIRECTORY_SEPARATOR;
        foreach ($extensions as $ext) {
            ...
            if (false !== $icon) {
                ...
                if ($set === 'svg' && ($ext === 'mp4' || $ext === 'exe') && file_exists($newIconPath) && !file_exists($defaultIconPath . $ext . '.' . preg_replace('/[0-9]+/', '', 'svg'))) {
                    file_put_contents($defaultIconPath . $ext . '.' . preg_replace('/[0-9]+/', '', 'svg'), file_get_contents($newIconPath));
                }
        ...
        // Reset svg flag
        update_option('wpfd_svg_icons_reset', true);
    ...
    public function saveSingleParams()
    {
        ...
        if (!$result) {
            $this->exitStatus(false, array('message' => esc_html__('Params saved but CSS file can not render!', 'wpfd')));
        }
