...
class WpfdHelperDocument
{
    ...
    public function getContent()
    {
        ...
        switch (mb_detect_encoding($content, $encodes)) {
            case 'ISO-8859-1':
                // phpcs:ignore Generic.PHP.DeprecatedFunctions.Deprecated -- Keep this
    ...
    private function cleanChar($content = '')
    {
        ...
        /**
         * Filter to clean chars or not to avoid some search keys not work as expected
         *
         * @param bool Process
         *
         * @return bool
         ...
         * @internal
         */
        $cleanCharProcess = apply_filters('wpfd_plain_text_search_clean_char_processing', true);
        ...
        if ($cleanCharProcess) {
            ...
            /**
             * Filter to change the char of document cleaner
             *
             * @param array Chars
             *
             * @return array
             ...
             * @internal
             */
            $punct = apply_filters('wpfd_plain_text_search_clean_chars', $punct);
            $punct_to_exclude = apply_filters('wpfd_plain_text_search_punct_to_exclude', array());
            $punct = array_diff($punct, $punct_to_exclude);

            $hyphenSplit = apply_filters('wpfd_fts_split_hyphen', true);
            if (!$hyphenSplit) {
                $punct = array_diff($punct, array('-'));
            }

            $content = str_replace($punct, ' ', $content);
            $pattern = '/[[:punct:]';
            foreach ($punct_to_exclude as $char) {
