...
class WpfdHelperFiles
{
     ...
     * @param string  $fileDownloadUrl File download url
     * @param string  $fileExt         File type
     * @param string  $fileUploadDate  File upload date
     *
     * @return void
     */
    public static function sendMail($action, $receiver, $cat_title, $website_url, $file_title, $fileDownloadUrl = '#', $fileExt = '', $fileUploadDate = '')
    {
        ...
        // Replace email subject content
        $subject = self::emailSubjectReplace(
            $subject,
            $cat_title,
            $website_url,
            $file_title,
    ...
    public static function emailBodyReplace(
        ...
        $email_body = str_replace('{file_name}', $file_title . '.' . $fileExt, $email_body);
        $email_body = str_replace('{username}', $username, $email_body);
        $email_body = str_replace('{receiver}', $receiver, $email_body);
        $email_body = str_replace('{file_download_url}', esc_url($fileDownloadUrl), $email_body);
        $email_body = str_replace('{date_upload}', $fileUploadDate, $email_body);
     ...
     * @return mixed
     */
    public static function emailSubjectReplace(
        $emailSubject,
        $cat_title,
        $website_url,
        $file_title,
        ...
        $emailSubject = str_replace('{category}', $cat_title, $emailSubject);
        $emailSubject = str_replace('{website_url}', $website_url, $emailSubject);
        $emailSubject = str_replace('{file_name}', $file_title, $emailSubject);
        $emailSubject = str_replace('{username}', $username, $emailSubject);
        $emailSubject = str_replace('{receiver}', $receiver, $emailSubject);
